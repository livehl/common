...
import scala.collection.{IterableLike, Parallelizable}
...
object Tool {
  ...
  //带重试的区段
  def reTry(count: Int = 5,sleep:Int=50)(f: => Unit) {
    var retryCount = 0
...
//        e.printStackTrace()
  ...
  def getSettingMap(): Map[String, String] = {
    //这里取数据有一秒缓存延迟
    getCache("setting").asInstanceOf[Map[String, String]]
  }
