...
object NetTool {
  ...
  private def getHttpClient(useConnmanager: Boolean = true) = {
  ...
  def wrapClient(base:CloseableHttpClient):CloseableHttpClient=
  {
    try {
      ...
      val sslSF = new SSLConnectionSocketFactory(ctx, SSLConnectionSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER);
